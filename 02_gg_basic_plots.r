...
ggplot(aes(x = XXXX, y = XXXX), data = summary2005) + 
  geom_point()

## Scatter Plot + regression line ###----
# Hint: Use the cheat sheet to find the right geom
ggplot(aes(x = population, y = stores), data = summary2005) + 
  geom_point() + 
  geom_XXXX() # Replace the XXXX

### Bubble Chart ###----
ggplot(aes(x = population, y = stores, XXXX = market_share), data = summary2005) +
  geom_point()

### Bar chart - Unordered ###----
ggplot(aes(x = state, y = population), data = summary2005) + 
